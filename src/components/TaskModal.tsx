      ...
      >
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          <DatePickerField
            value={todo.due_date}
            onChange={(due_date) =>
              // Clearing the date clears any time along with it — due_time
              // can't outlive its due_date (same invariant TaskComposer's
              // create-flow enforces).
              onUpdate(todo.id, due_date ? { due_date } : { due_date, due_time: null, duration_minutes: null })
            }
